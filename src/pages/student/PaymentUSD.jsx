...
import { ArrowLeft, Lock, CheckCircle, Tag, X } from 'lucide-react';
...
const translations = {
  ...
  ar: {
    ...
    securePaymentWithPaypal: 'دفع آمن عبر PayPal',
    paypalRedirect: 'سيتم تحويلك لـ PayPal لإكمال الدفع.',
    paypalIntegrationArea: 'تكامل PayPal',
    paypalIntegrationDescription: 'يُعرض هنا زر/تدفق PayPal بعد التكامل.',
    implementationSteps: 'خطوات التنفيذ:',
    acceptedPaymentMethods: 'طرق الدفع:',
    creditDebitCards: '💳 بطاقات',
    cardsDescription: 'Visa، Mastercard',
    paypalBalance: '💰 رصيد PayPal',
    paypalBalanceDescription: 'الدفع من الرصيد',
    yourPaymentIsSecure: 'دفعتك مؤمّنة',
    secureDescription: 'PayPal يعالج المعاملات بأمان. لا نخزّن بيانات البطاقة.',
    ...
    subtotal: 'الفرعي',
    processingFee: 'رسوم',
    processingFeeNote: '(٢٫٩٪ + ٠٫٣٠ دولار)',
    total: 'الإجمالي',
    whatYoullGet: 'ما ستحصلين عليه:',
    jobApplicationsPerMonth: '١٠٠ طلب/شهر',
    priorityInSearch: 'أولوية في البحث',
    featuredProfileBadge: 'شارة',
    prioritySupport: 'دعم أولوية',
    seeJobBudgets: 'رؤية الميزانيات',
    paypalIntegrationPending: 'تكامل قيد الانتظار',
    paypalButtonWillAppear: 'سيظهر زر PayPal هنا',
    securePaymentPoweredBy: 'دفع عبر PayPal',
    pleaseEnterCouponCode: 'أدخلي كود الكوبون',
    invalidCouponCode: 'كود غير صالح',
    paypalIntegrationComingSoon: 'تكامل PayPal قريباً',
    paymentProcessingError: 'خطأ في المعالجة',
...
const PaymentUSD = () => {
  ...
  const { amount, billingCycle = 'monthly' } = location.state || {};
  const currency = 'USD';
  ...
  const upgradeMutation = useMutation({
    ...
    onError: (error) => {
      ...
      if (isPayPalConfig) {
        alert(t.paymentProcessingError + ' (PayPal is not configured on the server.)');
      } else {
        alert(message);
      }
    },
        ...
        <div className="lg:col-span-2">
          <Card title={t.completePayment}>
            <div className="space-y-6">
              ...
              <p className="text-sm text-gray-600">
                Click &quot;Pay with PayPal&quot; below to complete your subscription. You will be redirected to PayPal to pay securely.
              </p>
